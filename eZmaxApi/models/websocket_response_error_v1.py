...
class WebsocketResponseErrorV1(BaseModel):
    """
    Response for Websocket Error V1  # noqa: E501
    """
    e_websocket_messagetype: StrictStr = Field(..., alias="eWebsocketMessagetype", description="The Type of message")
    m_payload: WebsocketResponseErrorV1MPayload = Field(..., alias="mPayload")
    ...
    def e_websocket_messagetype_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('Response-Error-V1'):
            raise ValueError("must be one of enum values ('Response-Error-V1')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> WebsocketResponseErrorV1:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> WebsocketResponseErrorV1:
        """Create an instance of WebsocketResponseErrorV1 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return WebsocketResponseErrorV1.parse_obj(obj)

        _obj = WebsocketResponseErrorV1.parse_obj({
            "e_websocket_messagetype": obj.get("eWebsocketMessagetype"),
            "m_payload": WebsocketResponseErrorV1MPayload.from_dict(obj.get("mPayload")) if obj.get("mPayload") is not None else None
        })
        return _obj
